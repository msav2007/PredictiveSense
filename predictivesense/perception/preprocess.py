...
class LetterboxResult:
    """``blob`` is NCHW float32 RGB in [0, 1]; ``ratio`` and the pads undo it."""
...
def letterbox(image: np.ndarray, size: int) -> LetterboxResult:
    """Aspect-preserving resize of a BGR ``HxWx3`` uint8 image to ``size x size``.

    Returns a contiguous ``(1, 3, size, size)`` float32 RGB blob normalised to
    [0, 1] plus the ratio and pad offsets needed to map model-space boxes back.
    """
