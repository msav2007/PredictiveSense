...
- **2026-09-07** Every duration uses `time.monotonic()` (never `time.time()` and, per the prompt's "monotonic() only", not `time.perf_counter()` either). On CPython 3.11 Windows `time.monotonic()` is `QueryPerformanceCounter`-backed, so sub-millisecond iteration latencies are still resolved. `datetime.now(timezone.utc)` is used only for wall-clock timestamps in the manifest and CSV, not for measuring elapsed time.
